...
class ProductList extends Component{
  ...
  render(){
    ...
    return(
      ...
      {servicesCategories.map(service => <NavLink className="btn small-btn btn-outlined" activeClassName="link-active" to={`/catagoryid/${service.id}/${service.category}`} > {service.heading} </NavLink>  )}
          </div>
        ...
        <div className="row aim-subheading">
          ...
          <div className="row product-list-box">
            { productsList.map( product => product.category === this.props.params.category && <ProductCard product={product} /> ) }
          </div>
        </div>
